...
gulp.task('concat-css', function(){
    return gulp.src(['source/css/*.css']) //ojo el orden afecta usar un arreglo
...
gulp.task('concat-files', function(){
    gulp.src('source/js/*.js')    //toma los nombres de los js en la pcarpeta source
        .pipe(concat('lab33.min.js'))  //concatena los nombre con la palabra min
        .pipe(uglify())     // crea los .min
        .pipe(gulp.dest('dist/')); //guarda los .min en la carpeta build
});
