$(document).on('pageinit', '#add-contact', function(){ //evednto de inicio

    console.log('Evento delegado `#add-contact`');

    $('#save-contact').on('click', function(){   // llamada con clic
        ...
        function onSuccess(contact){
            ...
            navigator.notification.confirm(
                '¡Hola!, creaste el contacto: ' + contact.name.givenName + ' ' + contact.name.familyName,
                onConfirm,
        ...
        var contact = navigator.contacts.create();  //contacts api de cordova

        contact.displayName = $('#first-name').val();

        contact.nickName = $('#first-name').val();

        var name = new ContactName();  //crear contacto
        ...
        emails[0] = new ContactField('personal', email, true);  //crear cooreo arreglo
        ...
        mobile_numbers[0] = new ContactField('mobile', mobile, true); //crear numero de telefono

        contact.phoneNumbers = mobile_numbers;

        console.log(contact);

        contact.save(onSuccess, onError);  //guardar los datos
    });
});